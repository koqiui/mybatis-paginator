package com.github.ext.mybatis.paginator.dialect;
...
import com.github.ext.mybatis.paginator.domain.OrderItem;
...
public class Dialect {
	...
	protected void init() {
		...
		pageSQL = sql;
		if (pageBounds.getOrderItems() != null && !pageBounds.getOrderItems().isEmpty()) {
			pageSQL = getSortString(sql, pageBounds.getOrderItems());
		}
	 ...
	 * @return 总记录数的sql
	 */
	protected String getSortString(String sql, List<OrderItem> ordersItems) {
		...
		for (OrderItem orderItem : ordersItems) {
			if (orderItem != null) {
				buffer.append(orderItem.toString()).append(", ");
			}
